...
$sourceScript = Join-Path $workspace ".rpi-provisioning\private\force-vivo-wifi.sh"
$statusPath = Join-Path $workspace ".rpi-provisioning\force-vivo-injection-status.json"
$transcriptPath = Join-Path $workspace ".rpi-provisioning\force-vivo-injection.log"
...
try {
    if (-not (Test-Path -LiteralPath $sourceScript -PathType Leaf)) {
        throw "The prepared force-Vivo script is missing."
    }
    ...
    if ($sourceHash -ne $targetHash) {
        throw "The force-Vivo script failed verification."
    }
    ...
    Set-InjectionStatus -Phase "failed" -ExitCode 1 -Message $message
    Write-Error $message
    exit 1
}
finally {
